...
for s in range(nb_seeds):
    for s in i_strategies:
        job_file = os.path.join(job_directory, f"test_strategy={s}%.slurm")

        with open(job_file, 'w') as fh:
            fh.writelines("#!/bin/bash\n")
            fh.writelines("#SBATCH --account=kcr@v100\n")
            fh.writelines(f"#SBATCH --job-name=test_strategy={s}\n")
            fh.writelines("#SBATCH --qos=qos_gpu-t3\n")
            fh.writelines(f"#SBATCH --output=test_strategy={s}_%_%j.out\n")
            fh.writelines(f"#SBATCH --error=test_strategy={s}_%_%j.out\n")
            ...
            fh.writelines(f"srun python -u -B train.py --internalization-strategy {s} --save-dir 'test_strategy={s}/' 2>&1 ")
