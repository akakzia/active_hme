...
for s in range(nb_seeds):
    for q in queues:
        for beta in betas:
            job_file = os.path.join(job_directory, f"queries_q={q}_beta={beta}%.slurm")

            with open(job_file, 'w') as fh:
                fh.writelines("#!/bin/bash\n")
                fh.writelines("#SBATCH --account=kcr@v100\n")
                fh.writelines(f"#SBATCH --job-name=queries_q={q}_beta={beta}\n")
                fh.writelines("#SBATCH --qos=qos_gpu-t3\n")
                fh.writelines(f"#SBATCH --output=queries_q={q}_beta={beta}_%_%j.out\n")
                fh.writelines(f"#SBATCH --error=queries_q={q}_beta={beta}_%_%j.out\n")
